...
import { data } from '@/utils/data_file';
    ...
    <div className='grid_setup '>
      {
        data.map((eachPost: PostProps) =>(
          ...
          <div key={Math.random()} className='p-3 dark:bg-primary-dark-bg bg-white dark:text-white text-black rounded-xl'>
            <div>
              <h2 className='text-xl font-bold text-center'>{eachPost.title}</h2>
              <p className='text-14 dark:text-gray-400 text-slate-700 mt-2'>{eachPost.body}</p>
            </div>
          </div>
        ))
      }
